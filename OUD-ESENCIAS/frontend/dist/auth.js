...
function inicializarLogin() {
    ...
    formulario.addEventListener("submit", async (evento) => {
        ...
        const email = formulario.elements.namedItem("email").value.trim();
        const password = formulario.elements.namedItem("password").value;
        botón.disabled = true;
...
function inicializarRegistro() {
    ...
    formulario.addEventListener("submit", async (evento) => {
        ...
        const nombreCompleto = formulario.elements.namedItem("nombreCompleto").value.trim();
        const email = formulario.elements.namedItem("email").value.trim();
        const telefono = formulario.elements.namedItem("telefono").value.trim();
        const password = formulario.elements.namedItem("password").value;
        const confirmarPassword = formulario.elements.namedItem("confirmarPassword").value;
        const errorPassword = validarPasswordSegura(password);
        if (errorPassword)
            return mostrarError(cajaError, errorPassword);
        if (password !== confirmarPassword)
            return mostrarError(cajaError, "Las contraseñas no coinciden.");
        botón.disabled = true;
