 ...
 */
export async function agregarAlCarrito(perfumeId, cantidadDeseada = 1) {
    const perfume = await obtenerPerfumePorId(perfumeId);
    if (!perfume || !perfume.activo)
        return "Ese perfume ya no está disponible.";
...
export async function actualizarCantidad(perfumeId, nuevaCantidad) {
    const perfume = await obtenerPerfumePorId(perfumeId);
    if (!perfume)
        return "Ese perfume ya no está disponible.";
    const items = leerCarrito();
...
export function quitarDelCarrito(perfumeId) {
...
async function renderizarCarrito() {
    const lista = document.querySelector("[data-lista-carrito]");
    const vacio = document.querySelector("[data-carrito-vacio]");
    const totalesEl = document.querySelector("[data-carrito-totales]");
    if (!lista)
        return;
    const items = leerCarrito();
    lista.innerHTML = "";
...
function mostrarToast(mensaje, tipo = "exito") {
    const contenedor = document.querySelector("[data-toasts]");
    if (!contenedor)
        return;
...
function inicializarCupon() {
    const formulario = document.querySelector("[data-form-cupon]");
    if (!formulario)
        return;
    const mensaje = formulario.querySelector("[data-cupon-mensaje]");
    formulario.addEventListener("submit", async (evento) => {
        ...
        const input = formulario.elements.namedItem("codigoCupon");
...
function inicializarCheckout() {
    const boton = document.querySelector("[data-boton-checkout]");
    if (!boton)
        return;
...
async function abrirModalPedidos() {
    const sesion = obtenerSesionActual();
    const modal = document.querySelector("[data-modal-pedidos]");
    const listaPedidos = document.querySelector("[data-lista-pedidos]");
    if (!sesion || !modal || !listaPedidos)
        return;
    ...
    for (const pedido of pedidos) {
      ...
      <ul class="pedido-card__lineas">
        ${pedido.lineas
            .map((l) => `<li>${l.cantidad} × ${escaparHtml(l.nombre)} <span>${formatoMoneda(l.subtotal)}</span></li>`)
            .join("")}
      </ul>
...
function inicializarCarritoGlobal() {
    ...
    document.addEventListener("click", (evento) => {
        ...
        if (objetivo.closest("[data-abrir-pedidos]"))
            abrirModalPedidos();
    });
    inicializarCupon();
    inicializarCheckout();
